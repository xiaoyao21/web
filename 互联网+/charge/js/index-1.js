...
xhr.onreadystatechange = function() {
	if (xhr.readyState == 4) {
		if (xhr.status == 200) {
		  ...
		  if(json2.rows[i].orderDetails[0].goods.goodsType == 1){
				...
				for (var j = 1; j < json2.rows[i].orderDetails.length; j++) {
					...
					if(json2.rows[i].orderDetails[j].goods.goodsType == 1){
						k = 1;
			...
			if(json2.rows[i].orderDetails[0].goods.goodsType == 2){//第一个是水果
			var rows = 0;
			for (var j = 1; j < json2.rows[i].orderDetails.length; j++) {
				if(json2.rows[i].orderDetails[j].goods.goodsType == 1){
				rows++;//共几行
			...
			console.log(rows)
			var flag = 0;
			for (var j = 1; j < json2.rows[i].orderDetails.length; j++) {//从第二个开始算找到第一个是蔬菜的值
				if(flag == 0){	
				if(json2.rows[i].orderDetails[j].goods.goodsType == 1){
						flag = 1;
					 var arr =  fenge(json2.rows[i].orderDetails[j].goods.goodsImg)
				...
				<td class="kuaidi" rowspan=${rows}	>
				${arr1[1]}
				</td>
				<td class="zhuangtai" rowspan=${rows}>
					<center><p class="Status">${state}</p>
					<button class="status" id = "Status" >发货</button></center>
				</td>
...
xhr.open("GET", "http://120.79.141.139/MTmarket/orderApi/1/3", true);
xhr.send(null);
...
class SimplePagination {
	...
	switchPrevNextAble() {
		...
		xhr.open("GET", `http://120.79.141.139/MTmarket/orderApi/${state.pageNumber}/3`, true);
...
function xiangxi(canshu) {
	...
	for (let i = 0; i < canshu; i++) {
		pac_more[i].addEventListener('click', function() {
			...
			window.open("../html-vegetable/detail.html?index=" + index + "&curpage=" + curpage);
