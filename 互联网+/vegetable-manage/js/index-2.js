...
xhr.onreadystatechange = function() {
	if (xhr.readyState == 4) {
		if (xhr.status == 200) {
		  ...
		  if(json2.rows[i].orderDetails[0].goods.goodsType == 2){
				...
				for (var j = 1; j < json2.rows[i].orderDetails.length; j++) {
					if(json2.rows[i].orderDetails[j].goods.goodsType == 2){
...
class SimplePagination {
	...
	switchPrevNextAble() {
		...
		if(json2.rows[i].orderDetails[0].goods.goodsType == 1){//第一个是水果
			var rows = 0;
			for (var j = 1; j < json2.rows[i].orderDetails.length; j++) {
				if(json2.rows[i].orderDetails[j].goods.goodsType == 2){
				rows++;//共几行
...
function xiangxi(canshu) {
	...
	for (let i = 0; i < canshu; i++) {
		pac_more[i].addEventListener('click', function() {
			...
			window.open("../html-fruit/detail-2.html?index=" + index + "&curpage=" + curpage);
