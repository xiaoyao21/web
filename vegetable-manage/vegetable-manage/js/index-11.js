...
xhr.open("GET", "http://120.79.141.139/MTmarket/orderApi/1/1/3", true);
xhr.send(null);
...
class SimplePagination {
	...
	switchPrevNextAble() {
		...
		xhr.open("GET", `http://120.79.141.139/MTmarket/orderApi/1/${state.pageNumber}/3`, true);
...
function xiangxi(canshu) {
	...
	for (let i = 0; i < canshu; i++) {
		pac_more[i].addEventListener('click', function() {
			...
			window.open("../html-student/detail-no.html?index=" + index + "&curpage=" + curpage);
